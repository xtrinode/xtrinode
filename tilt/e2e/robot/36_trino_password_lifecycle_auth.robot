...
Apply Password Lifecycle XTrinode
    ...
    ${json}=    Set Variable    {"apiVersion":"analytics.xtrinode.io/v1","kind":"XTrinode","metadata":{"name":"${AUTH_XTRINODE}","namespace":"${AUTH_NAMESPACE}","labels":{"test.xtrinode.io/contract":"password-lifecycle-auth"}},"spec":{"size":"xs","minWorkers":1,"maxWorkers":1,"suspended":false,"autoSuspendAfter":"30m","resources":{"coordinator":{"requests":{"cpu":"250m","memory":"1Gi"},"limits":{"cpu":"1","memory":"1536Mi"}},"worker":{"requests":{"cpu":"250m","memory":"1Gi"},"limits":{"cpu":"1","memory":"1536Mi"}}},"trinoControlAuth":{"username":"${AUTH_CONTROL_USER}","passwordSecret":{"name":"${AUTH_CONTROL_SECRET}","key":"password"}},"routing":{"header":"X-Trino-XTrinode=${AUTH_XTRINODE}","routingGroup":"${AUTH_XTRINODE}"},"valuesOverlay":{"image":{"repository":"${TRINO_IMAGE_REPOSITORY}","tag":"${TRINO_IMAGE_TAG}","pullPolicy":"IfNotPresent"},"server":{"config":{"authenticationType":"PASSWORD"}},"auth":{"passwordAuth":"${AUTH_CONTROL_USER}:${AUTH_PASSWORD_HASH}"},"coordinator":{"startupProbe":{"exec":{"command":["/usr/lib/trino/bin/health-check"]},"initialDelaySeconds":10,"periodSeconds":5,"timeoutSeconds":3,"failureThreshold":90}},"worker":{"gracefulShutdown":{"enabled":true,"gracePeriodSeconds":5},"startupProbe":{"exec":{"command":["/usr/lib/trino/bin/health-check"]},"initialDelaySeconds":10,"periodSeconds":5,"timeoutSeconds":3,"failureThreshold":90}},"additionalConfigProperties":["internal-communication.shared-secret=${AUTH_INTERNAL_SECRET}","query.max-memory=512MB","query.max-memory-per-node=384MB","memory.heap-headroom-per-node=256MB"]}}}
